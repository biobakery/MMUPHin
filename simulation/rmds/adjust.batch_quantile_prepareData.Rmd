...
```
```{r load normalize and write otu table}
dir.create(paste0(dir_output, "otu_table/"), showWarnings = TRUE, recursive = TRUE)
results <- foreach(i = 1:N) %dopar% {
  mat_otu <- read.table(paste0(dir_data,
                               "sparseDOSSA_sets/",
                               i, ".tsv"),
                        header = TRUE,
                        sep = "\t",
 ...
 mat_otu <- mat_otu %>% 
   apply(2, MMUPHin:::tss)
 mat_otu %>% data.frame() %>% 
    cbind(data.frame(Features = rownames(mat_otu)), .) %>% 
    readr::write_tsv(path = paste0(dir_output, "otu_table/", i, ".tsv"))
  return(NULL)
}
```
