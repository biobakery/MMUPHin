...
l_continuous_before <- foreach(i = 1:N) %dopar% {
  ...
  phylo <- phyloseq::phyloseq(
    phyloseq::otu_table(i_result$otu_count[[1]], taxa_are_rows = TRUE),
  ...
  ggsave(filename = paste0(paste0(dir_output, i, "/"), "ordination_",
         ...
         width = 3*i_result$nBatch, height = 6, limitsize = FALSE)
  fit_continuous <- MMUPHin::continuous.discover(
  ...
  if(is.null(fit_continuous)) {
    return(fit_continuous)} else {
      scores <- t(asin(sqrt(apply(i_result$otu_count[[1]], 2, MMUPHin:::tss)))) %*% 
...
#          cowplot::plot_grid(p1, p2, nrow = 2),
#          width = 3*i_result$nBatch, height = 6, limitsize = FALSE)
#   fit_continuous <- MMUPHin::continuous.discover(feature.count = i_result$otu_count[[1]] %>% apply(2, MMUPHin:::tss),
#                                                  batch = "batch",
#                                                  data = df_metadata,
#                                                  transform = "AST",
#                                                  var.perc.cutoff = 0.5,
...
#                                                  verbose = FALSE)
#   if(is.null(fit_continuous)) {
#     l_continuous_before[[i]] <- NULL} else {
#     scores <- t(asin(sqrt(apply(i_result$otu_count[[1]], 2, MMUPHin:::tss)))) %*% 
#       fit_continuous$consensus.loading
#     l_continuous_before[[i]] <- cor(scores, df_metadata[, c("PC1", "PC2")])
#   }
# }
