...
metadata_type <- c("batch" = "factor",
                   "score" = "numeric",
                   "score_neg" = "numeric")
tb_effectSize <- tidyr::crossing(batch = c(0, 1, 2, 5, 10),
                                 score = c(2, 5, 10)) %>% 
  dplyr::mutate(score_neg = score)
...
tb_simSetup <- tidyr::crossing(
  ...
  dplyr::mutate(nSample = nSample_perBatch*nBatch)
```
...
tb_sim <- tb_simSetup %>% 
  ...
  dplyr::mutate(df_metadata = list(
    data.frame(batch = rep(1:nBatch, each = nSample_perBatch) %>%
      ...
      dplyr::mutate(score_neg = -score) %>% 
      data.frame(row.names = paste0("Sample", 1:nSample))),
