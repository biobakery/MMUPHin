...
l_permanova_before <- foreach(i = 1:nrow(tb_results)) %dopar% {
  i_result <- tb_results[i, ]
  distance <- i_result$otu_count[[1]] %>% 
    phyloseq::otu_table(taxa_are_rows = TRUE) %>% 
    phyloseq::transform_sample_counts(MMUPHin:::tss) %>% 
    phyloseq::distance(method = "bray")
  list(batch = try(vegan::adonis(distance ~ batch, 
                             data = i_result$df_metadata[[1]],
                             permutations = 2),
                   silent = TRUE),
       positive_control = try(vegan::adonis(distance ~ positive_control, 
                                        data = i_result$df_metadata[[1]],
                                        permutations = 2),
                              silent = TRUE),
       negative_control = try(vegan::adonis(distance ~ negative_control, 
                                        data = i_result$df_metadata[[1]],
                                        permutations = 2),
                              silent = TRUE))
}
...
```{r batch adjustment}
registerDoParallel(15)
l_adjust.batch <- foreach(i = nrow(tb_results)) %dopar% {
  i_result <- tb_results[i, ]
  colnames(i_result$otu_count[[1]]) <- 
    rownames(i_result$df_metadata[[1]]) <- 
    paste0("Sample", 1:i_result$nSample)
  otu_adjust <- MMUPHin::adjust.batch(
    feature.count = i_result$otu_count[[1]],
    batch = "batch",
    covariates = c("positive_control", "negative_control"),
    data = i_result$df_metadata[[1]],
    zero.inflation = TRUE,pseudo.count = 0.5,
    diagnostics = FALSE,
    verbose = FALSE
  )
...
registerDoParallel(15)
l_permanova_after <- foreach(i = 1:nrow(tb_results)) %dopar% {
  i_result <- tb_results[i, ]
  distance <- l_adjust.batch[[i]]$otu_adjust %>% 
    phyloseq::otu_table(taxa_are_rows = TRUE) %>% 
    phyloseq::transform_sample_counts(MMUPHin:::tss) %>% 
    phyloseq::distance(method = "bray")
...
registerDoParallel(15)
l_permanova_afterComBat <- foreach(i = 1:nrow(tb_results)) %dopar% {
  i_result <- tb_results[i, ]
  distance <- l_adjust.batch[[i]]$otu_adjust_ComBat %>% 
    phyloseq::otu_table(taxa_are_rows = TRUE) %>% 
    phyloseq::transform_sample_counts(MMUPHin:::tss) %>% 
    phyloseq::distance(method = "bray")
  list(batch = vegan::adonis(distance ~ batch, 
