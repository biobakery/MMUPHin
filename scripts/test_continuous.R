...
feature.count <- qe_biom_genus %>% otu_table() %>% `@`(`.Data`)
batch <- batch %>%
  gsub(".metaphlan_bugs_list.stool", "", ., fixed = TRUE)
...
log.data <- log(apply(feature.count + pseudo.count,
                      2,
                      function(x) x / sum(x)))
log.data <- asin(sqrt(apply(feature.count,
                            2,
                            function(x) x / sum(x))))
...
pc.graph.tmp <- pc.graph %>%
  igraph::delete.vertices(
    igraph::V(pc.graph)[igraph::membership(pc.cluster) %in%
                  (names(igraph::sizes(pc.cluster))[igraph::sizes(pc.cluster) == 1] %>% as.numeric)]
  )
list.membership.tmp <- igraph::membership(pc.cluster)[names(igraph::V(pc.graph.tmp))]
list.membership.tmp <- unique(list.membership.tmp) %>%
  lapply(function(x) names(list.membership.tmp)[list.membership.tmp == x])
igraph::plot.igraph(pc.graph.tmp, mark.groups = list.membership.tmp,
            vertex.size = igraph::degree(pc.graph.tmp) / max(igraph::degree(pc.graph.tmp)) * 15,
            edge.width = igraph::E(pc.graph.tmp)$weight * 10)
...
mat_tax <- tax_table(qe_biom_genus)@.Data %>%
  data.frame() %>%
  rownames_to_column("otu")
tb_cons <- data.frame(mat.cons.loading) %>%
  rownames_to_column("otu")
p1 <- tb_cons %>% arrange(desc(abs(X1))) %>%
  slice(1:10) %>%
  ggplot(aes(x = otu,
             y = X1)) +
...
p2 <- tb_cons %>% arrange(desc(abs(X5))) %>%
  slice(1:10) %>%
  ggplot(aes(x = otu,
             y = X5)) +
  geom_bar(stat = "identity") +
...
p2 <- tb_cons %>% arrange(desc(abs(X2))) %>%
  slice(1:10) %>%
  ggplot(aes(x = paste0(Rank2, ";", Rank5, ";", Rank6),
             y = X2)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_bw()
p3 <- tb_cons %>% arrange(desc(abs(X3))) %>%
  slice(1:10) %>%
  ggplot(aes(x = paste0(Rank2, ";", Rank5, ";", Rank6),
             y = X3)) +
  geom_bar(stat = "identity") +
